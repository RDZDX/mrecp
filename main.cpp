    ...
    if (port == -3) {
        ...
        if (vm_string_equals_ignore_case(text22, "echo") == 0 && strlen(text33) == 0) {
            ...
            if (vm_file_get_attributes(fullPath2) != -1) {
               ...
               //vm_file_delete(fullPath2); // create new text file text222 and write text444
               vm_ucs2_to_ascii(text, wstrlen(fullPath2) + 1, fullPath2);
               sprintf(text, "%s %s %s", text333, text222, text444);
               //strcat(text, " - OK");
               strcat(text, "\n\n");
               console_str_in(text);
           }
            ...
            //if (vm_file_get_attributes(fullPath2) != -1 || vm_file_get_attributes(fullPath2) != VM_FS_ATTR_DIR) {
            if (vm_file_get_attributes(fullPath2) != -1) {
               //vm_file_delete(fullPath2);  // open existing text file text222 and append text444
               vm_ucs2_to_ascii(text, wstrlen(fullPath2) + 1, fullPath2);
               sprintf(text, "%s %s %s", text333, text222, text444);
               //strcat(text, " - OK");
               strcat(text, "\n\n");
...
VMINT parseText2(VMSTR text) {
    ...
    if (strlen(text44X) > 0) {
       ...
       strncpy(text44Z, text44Y, strlen(text44Y) - 4);
       ptr3 = text44Z;
       stringReverse(ptr3);
       sprintf(text444, "%s", ptr3);
    }
