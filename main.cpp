...
VMCHAR text6[220];
VMCHAR text2[91] = " is not recognized as an internal or external command, operable program or batch file.\n\n";
...
VMCHAR text7[43] = "The system cannot find the path specified.";
VMCHAR text8[43] = "The system cannot find the file specified.";
...
void trim_left_symbolsx(char *reslt_data, char *inp_data);
void mid(const char *src, VMINT start, VMINT length, char *dst, VMINT dstlen);
...
void prompt(int tid) {
    if (port == -3) {
        ...
        if (vm_string_equals_ignore_case(text22, "echo") == 0 && strlen(text33) != 0) {
            ...
            strcpy(text, text8);
            vm_ascii_to_ucs2(texty, (strlen(text33) + 1) * 2, text33);
            vm_wstrcpy(fullPath2, fullPath1);
            vm_wstrcat(fullPath2, texty);
        ...
        // dir directoryname
            ...
            strcpy(text, text7);
            trim_left_symbols(text12, text33);
            strcat(text12, "\\");
            vm_ascii_to_ucs2(texty, (strlen(text12) + 1) * 2, text12);
            vm_wstrcpy(fullPath2, fullPath1);
            vm_wstrcat(fullPath2, texty);
            ...
            trim_left_symbols(text12, text33);
            strcat(text12, "\\");
            vm_ascii_to_ucs2(fullPath2, (strlen(text12) + 1) * 2, text12);
        ...
        } else if (vm_string_equals_ignore_case(text22, "test") == 0 &&  strlen(text33) != 0 && strlen(text44) != 0 && strlen(text55) == 0) { //newcopy
            trim_left_symbolsx(my_path, text44);
            lenght_555 = strlen(my_path); 
            lenght_666 = strlen(text44);
       
            if (lenght_555 == 0) {       //turim tik failovarda
                                        //tikriname ar einamajame kataloge nera katalogo/failo tokiu pavadinimu
                                        //paleidziam kopijavimo procedura

               //create_search_path(fullPath2, fullPath1, text33); //suformuojam source
               //if (vm_file_get_attributes(fullPath2) != -1) {
               //   create_search_path(fullPath3, fullPath1, text44); //suformuojam destination
               //   if (vm_file_get_attributes(fullPath3) == -1) {
               //      vm_file_copy(fullPath3, fullPath2, cb);
               //      vm_ucs2_to_ascii(text, wstrlen(fullPath3) + 1, fullPath3);
               //      strcat(text, " - OK");
               ...
               strcat(text, "\n");
               console_str_in(text);
               strcpy(my_file_name, text44);
               strcat(my_file_name, "\n\n");
               console_str_in(my_file_name);
               strcpy(my_file_name, "");

            } else if (lenght_666 == lenght_555) {        // turim tik kelia
                                        //turim tik katalogo kelia
                                        //tikriname katalogo kelia
                                        //jei katalogo kelias blogas iseinam
                                        //jei katalogo kelias geras tikrinam failo varda ar nurodytame kataloge nera failo/katalogo tokiu vardu

               //vm_ascii_to_ucs2(fullPath3, (strlen(my_path) + 1) * 2, my_path); //suformuojam destination path
               //if (vm_file_get_attributes(fullPath3) == VM_FS_ATTR_DIR) {       //tikrinam ar egzistuoja katalogas
               //create_search_path(fullPath4, fullPath3, text33);                //suformuojam destination path + filename
               //if (vm_file_get_attributes(fullPath4) == -1) {                   //tikrinam kad nebutu destination failo/katalogo tokiu vardu
               //create_search_path(fullPath2, fullPath1, text33);                //suformuojam source path + filename
               //if (vm_file_get_attributes(fullPath2) != -1) {                   //tikrinam ar egzistuoja source failas
               //vm_file_copy(fullPath3, fullPath2, cb);                          //kopijuojam copy text.txt e:/mre/

               strcpy(text, text33);
               strcat(text, "\n");
               console_str_in(text);
               strcat(my_path, "\n\n");
               console_str_in(my_path);
               strcpy(my_path, "");

            } else {
                                        //turim varianta katalogo kelias ir failo vardas
                                        //atskiriam katalogo kelia nuo failo vardo
                                        //bet tikriname katalogo kelia
                                        //jei katalogo kelias blogas iseinam
                                        //jei katalogo kelias geras tikrinam failo varda ar nurodytame kataloge nera failo/katalogo tokiu vardu

               mid(text44, lenght_555, lenght_666 - lenght_555, my_file_name, sizeof(my_file_name)); //atskiriam failo varda

               //vm_ascii_to_ucs2(fullPath3, (strlen(my_path) + 1) * 2, my_path); //suformuojam destination path
               //if (vm_file_get_attributes(fullPath3) == VM_FS_ATTR_DIR) {       //tikrinam ar egzistuoja katalogas
               //create_search_path(fullPath4, fullPath3, my_file_name);          //suformuojam destination path + filename
               //if (vm_file_get_attributes(fullPath4) == -1) {                   //tikrinam kad nebutu destination failo/katalogo tokiu vardu
               //create_search_path(fullPath2, fullPath1, text33);                //suformuojam source path + filename
               //if (vm_file_get_attributes(fullPath2) != -1) {                   //tikrinam ar egzistuoja source failas
               //vm_file_copy(fullPath3, fullPath2, cb);                          //kopijuojam copy text.txt e:/mre/
...
void mid(const char *src, VMINT start, VMINT length, char *dst, VMINT dstlen){
     ...
     VMINT len = min(dstlen - 1, length);
     strncpy(dst, src + start, len);
     if(len < length){dst[dstlen-1] = 0;}
}