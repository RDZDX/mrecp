...
char *test2;
char *test3;
VMCHAR text2[92] = " is not recognized as an internal or external command, operable program or batch file.\n\n";
VMCHAR text3[41] = "MRE console primitive [Version ALPHA].\n";
VMCHAR text4[150] = "about        - show program information\nhelp         - list available commands\necho         - print string\ndir          - list directory content\n";
VMCHAR text5[146] = "cd           - change current directory\ntype         - displays file contents\ncls          - clear screen\nexit         - quits the program\n\n";
VMCHAR text7[47] = "The system cannot find the path specified.";
VMCHAR text8[47] = "The system cannot find the file specified.";
VMCHAR text6[220];
VMCHAR text9[100];
VMINT hnd;
VMINT test;
...
void create_search_path(VMWSTR result, VMWSTR source);
char *trimLeft(char *string);
void catFile(VMSTR *file_path);
...
void prompt(int tid) {
	...
	if (vm_string_equals_ignore_case(text1, "echo ") == 0) {
	...
	} else if (vm_string_equals_ignore_case(text1, "type ") == 0) {

            test2 = text11; // pilnas teksatas "cat test.txt"
	    test3 = strsubstr(test2, 5, strlen(test2) - 5); //tekstas be pradzios "test.txt"
	    strcpy(text, text8); //"The system cannot find the path specified."
            //test3 = trimLeft(test3); // pravalom slesho simboli gale
            //strcat(test3, "\\");
	    vm_ascii_to_ucs2(texty, (strlen(test3) + 1) * 2, test3);
            vm_wstrcpy(fullPath2, fullPath1);
	    vm_wstrcat(fullPath2, texty);
	    hnd = vm_find_first_ext(fullPath2, &fileInfo);

	    if (hnd > 0 && fileInfo.attributes != VM_FS_ATTR_DIR && fileInfo.attributes != VM_FS_ATTR_VOLUME) {
                vm_ucs2_to_ascii(text, wstrlen(fullPath1) + 1, fullPath1);
	        vm_ucs2_to_ascii(text9, wstrlen(fileInfo.filefullname) + 1, fileInfo.filefullname);
                strcat(text, text9);
	    }
	    ...
	    sprintf(text, "%s%s", text3, "\n");
	    console_str_in(text);
	}

	else if (vm_string_equals_ignore_case(text11, "dir") == 0) {
	...
	else if (vm_string_equals_ignore_case(text11, "cd") == 0 || vm_string_equals_ignore_case(text11, "cd .") == 0) {
            strcpy(textx, "");
	    vm_ucs2_to_ascii(text, wstrlen(fullPath1) + 1, fullPath1);
	    strcat(text, "\n\n");
	    console_str_in(text);
	...
	else if (vm_string_equals_ignore_case(text11, "cd ..") == 0) {
            strcpy(textx, "");
	...
	else if (vm_string_equals_ignore_case(textx, "cd ") == 0) {
	    test2 = text11;
	    test3 = strsubstr(test2, 3, strlen(test2) - 3);
	    strcpy(text, text7);
            test3 = trimLeft(test3);
            strcat(test3, "\\");
	    vm_ascii_to_ucs2(texty, (strlen(test3) + 1) * 2, test3);
            vm_wstrcpy(fullPath2, fullPath1);
	    vm_wstrcat(fullPath2, texty);
	    ...
	    if (hnd > 0 && fileInfo.attributes == VM_FS_ATTR_DIR) {
	...
	else if (vm_string_equals_ignore_case(text11, "beep") == 0) {
...
void create_search_path(VMWSTR result, VMWSTR source) {
    ...
    vm_ascii_to_ucs2(wfile_extension, 8, "*.*");
    vm_wstrcpy(fullPathx1, source);
    vm_wstrcat(fullPathx1, wfile_extension);
...
void catFile(VMSTR file_path) {
    ...
    VMFILE f_read;
    VMINT ret;
    VMWCHAR cFile_path[100] = {};

    vm_ascii_to_ucs2(cFile_path, (strlen(file_path) + 1) * 2, file_path);
    f_read = vm_file_open(cFile_path, MODE_READ, FALSE);
