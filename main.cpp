...
VMINT rrrrrrr = 0;
VMINT page11 = 0;
VMINT plus_line = 29; //chimka trukstamu 29 pirmojo ekrano eiluciu skaiciaus kompensavimui bendrame kiekyje !
...
void prompt(int tid) {
    if (port == -3) {
        ...
        // dir directoryname
            ...
            console_str_in(text88);
            //if (vm_file_get_attributes(fullPath2) != -1 || vm_file_get_attributes(fullPath2) != VM_FS_ATTR_DIR) {vm_file_delete(fullPath2);}

        } else if (vm_string_equals_ignore_case(text22, "mkdir") == 0 &&  strlen(text33) == 0) {
            console_str_in(text88);
            //if (vm_file_get_attributes(fullPath2) == -1 {vm_file_mkdir(fullPath2);}

        } else if (vm_string_equals_ignore_case(text22, "rmdir") == 0 &&  strlen(text33) == 0) {
            console_str_in(text88);
            //if (vm_file_get_attributes(fullPath2) == VM_FS_ATTR_DIR) {vm_file_rmdir(fullPath2);}

        } else if (vm_string_equals_ignore_case(text22, "rename") == 0 &&  strlen(text33) == 0) {
            console_str_in(text88);
            //vm_file_rename(const VMWSTR filename, const VMWSTR newname);
            ...
            console_str_in(text88);
            //vm_file_copy(const VMWSTR dst, const VMWSTR src, vm_file_copy_cb cb);
