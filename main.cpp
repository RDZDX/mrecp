...
void create_app_txt_path1(VMWSTR text);
...
void vm_main(void){
	...
	//vm_reg_pen_callback(handle_penevt);
        ...
        create_app_txt_path1(fullPath3);
}
...
void prompt(int tid) {
	...
	if (vm_string_equals_ignore_case(text1, "echo ") == 0) {
	    ...
	    while (kounteris < 1000 || test > 0) {

	        test = vm_find_next_ext(hnd, &fileInfo);

                //if (test == 0) {vm_vibrator_once();}
                //if (test < 0) {vm_vibrator_once();}

                if (vm_wstrlen(fileInfo.filefullname) == 0) {break;}
...
//  VMWCHAR fullPath2[100];
    ...
    vm_ascii_to_ucs2(wfile_extension, 10, "\*.*");
    vm_wstrcat(fullPath1, wfile_extension);
    vm_wstrcpy(text, fullPath1);
