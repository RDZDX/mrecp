...
//void create_app_txt_path(VMWSTR text, const char* extt); // <- fix ???
...
//void create_search_path(VMWSTR result, VMWSTR source, const char* text) //<- fix ????
...
//void draw(){ //Cia piesia "_" kursoriu !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
void draw(){
    ...
    // Kursorių piešiame tik jei nevyksta scroll'inimas aukštyn
    if (myScroll == 0) {
        vm_graphic_line(
            layer_bufs[1],
...
void prompt(int tid) {
    if (port == -3) {
        ...
        if (vm_string_equals_ignore_case(text22, "echo") == 0 && strlen(text33) == 0) {
            ...
            console_str_in("last single quotes simbols from ' text ' was deleted\n");
            console_str_in("echo '' > file.txt create empty file file.txt\n");
            console_str_in("echo. text >> file.txt append to file\n");
            console_str_in("new line (CR) and text\n\n");
            //console_str_in("\n\n");
...
/*
void extract_path(char *reslt_data, const char *inp_data) {
    ...
    *reslt_data = '\0';
    ...
    reslt_data--;  // Move to the last character

    while (reslt_data != NULL) {
        if (*reslt_data == '\\') {
            reslt_data[1] = '\0';
            break;
        }
