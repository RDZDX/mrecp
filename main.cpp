...
VMCHAR text10[239] = "append       - source file text data co\n               py to destination file\nrename       - renames a file\nmove         - move a file\ndel          - deletes a file\ntype         - displays file contents\ncls          - clear screen\n";
VMCHAR text12[37] = "exit         - quits the program\n\n";
...
void prompt(int tid) {
    if (port == -3) {
        ...
        if (vm_string_equals_ignore_case(text22, "echo") == 0 && strlen(text33) == 0) {
            ...
            console_str_in("last single quotes simbol from ' text '\n");
            ...
            console_str_in("append       - joins two text files\n\n");
        ...
        // dir directoryname
         ...
         //append sourc_file dest_file
         //tikriname ar egzistuoja dest_file
         //tikriname ar dest_file nera katalogas
         //tikriname ar egzistuoja sourc_file
         //tikriname ar sourc_file nera katalogas

            extract_path(my_path, text44); // dest_file
            lenght_555 = strlen(my_path);
            lenght_666 = strlen(text44);
            strcat(text, "Source or destination file not found !\n\n");

            if (lenght_555 == 0) { // if not writed full path, only file name - tikriname ar egzistuoja source ir neegzistuoja destination
                create_search_path(fullPath2, fullPath1, text33); // sourc_file
                if (vm_file_get_attributes(fullPath2) != -1) { // if source file exists
                    create_search_path(fullPath3, fullPath1, text44);
                    if (vm_file_get_attributes(fullPath3) != -1) { //if destination file exists
                        //vm_file_copy(fullPath3, fullPath2, cb); // append data procedure
                        vm_ucs2_to_ascii(text, wstrlen(fullPath3) + 1, fullPath3);
                        strcat(text, " - OK");
                        strcat(text, "\n\n");
                    }
            ...
            } else if (lenght_666 == lenght_555) { // if not writed full path, only file name  - tikriname ar egzistuoja source ir neegzistuoja destination
                vm_ascii_to_ucs2(fullPath3, (strlen(my_path) + 1) * 2, my_path);
                if (vm_file_get_attributes(fullPath3) == VM_FS_ATTR_DIR) {
                    create_search_path(fullPath4, fullPath3, text33);  // sourc_file
                    if (vm_file_get_attributes(fullPath4) == -1) {  // if source file not exists
                        create_search_path(fullPath2, fullPath1, text33);  // sourc_file
                        if (vm_file_get_attributes(fullPath2) != -1) { // if source file exists
                            //vm_file_copy(fullPath4, fullPath2, cb);  // append data procedure
                            vm_ucs2_to_ascii(text, wstrlen(fullPath4) + 1, fullPath4);
                            strcat(text, " - OK");
                            strcat(text, "\n\n");
                        }
               ...
               strncpy(my_file_name, text44 + lenght_555, (lenght_666 - lenght_555));  // dest_file_name without full path - tikriname ar destination nera katalogas
                vm_ascii_to_ucs2(fullPath3, (strlen(my_path) + 1) * 2, my_path);
                if (vm_file_get_attributes(fullPath3) == VM_FS_ATTR_DIR) { // if dest_file_name is directory
                    create_search_path(fullPath4, fullPath3, my_file_name);
                    if (vm_file_get_attributes(fullPath4) == -1) { // if dest_file_name not exists
                        create_search_path(fullPath2, fullPath1, text33);
                        if (vm_file_get_attributes(fullPath2) != -1) { // if source_file_name exists
                            //vm_file_copy(fullPath4, fullPath2, cb);  // append data procedure
                            vm_ucs2_to_ascii(text, wstrlen(fullPath4) + 1, fullPath4);
                            strcat(text, " - OK");
                            strcat(text, "\n\n");
...
VMINT parseText2(VMSTR text) {
    ...
    while (*ptr != '\0') {
           ...
           if (*ptr == ' ' && counter != 2) {
              ...
              if (counter == 1) {strcpy(text22X, nauj_strng);}
              if (counter == 2) {strcpy(text33X, nauj_strng);}
              if (counter < 3) {strcpy(nauj_strng, "");}
              ptr++;
           }
    ...
    if (counter == 0) {strcpy(text22X, nauj_strng);}
    if (counter == 1) {strcpy(text33X, nauj_strng);}
    if (counter > 1) {strcpy(text44X, nauj_strng);}

      if (strlen(text22X) > 0) {
       ptr1 = text22X;
       stringReverse(ptr1);
       sprintf(text222, "%s", ptr1);
    }
    ...
    if (strlen(text44X) > 0) {
       ...
       if (strlen(text44Y) != strlen(text22)) {
          strncpy(text44Z, text44Y, strlen(text44Y) - (strlen(text22) + 1)); //nuimame ohce = echo
         ...
         if (strlen(text44K) > 0) {sprintf(text444, "%s", text44K);}
       }
    }
...
void trim_single_spec_symb(char *reslt_data, char *inp_data) {
    ...
    // from left ->
    while (*inp_data) { // paleidziam cikla per teksto ilgi nuo pirmo iki paskutinio simbolio
        if ('\'' != (unsigned char)*inp_data && flag == 0 || flag1 == 1) { // jei simbolis nelygu kabute
            *reslt_data++ = *inp_data; // perkeliam simboli i rezultato teksta
        }
        flag1 = 1;
        inp_data++;
    ...
    // from right <-
    reslt_data--;
    if ('\'' == (unsigned char)*reslt_data) {*reslt_data = '\0';}
    else {
       reslt_data++;
       *reslt_data = '\0';
    }
