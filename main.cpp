...
void prompt(int tid) {
    if (port == -3) {
        ...
        if (vm_string_equals_ignore_case(text22, "echo") == 0 && strlen(text33) == 0) {
            ...
            console_str_in("When redirecting to file by\n");
            console_str_in(">, first and last single\n");
            console_str_in("quotes simbol from text was\n");
            console_str_in("deleted\n\n");
...
VMINT parseText2(VMSTR text) {
    ...
    if (strlen(text44X) > 0) {
       ...
       strncpy(text44Z, text44Y, strlen(text44Y) - 5);
       ptr3 = text44Z;
       stringReverse(ptr3);
       sprintf(text44Q, "%s", ptr3);
       trim(text44Q, text44Q);
...
void trim_single_spec_symb(char *reslt_data, char *inp_data) {
    ...
    // from left ->
    while (*inp_data) {
        if ('\'' != (unsigned char)*inp_data && flag == 0 || flag1 == 1) {
            *reslt_data++ = *inp_data;
        }
        flag1 = 1;
        inp_data++;
    ...
    while (1) {
        reslt_data--;
        if ('\'' != (unsigned char)*reslt_data && flag == 0 || flag2 == 1) {
            break;
        }
        flag = 0;
        *reslt_data = '\0';
