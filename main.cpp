...
char *test2;
char *test3;
VMCHAR text2[100] = " is not recognized as an internal or external command, operable program or batch file.\n";
...
void prompt(int tid) {
	if (port == -3) {
		...
		//console_str_in("\nPlease enter the port: ");
                ...
                else if (vm_string_equals_ignore_case(ip, "exit") == 0)
                {
                   vm_exit_app();
                }
                else if (vm_string_equals_ignore_case(ip, "help") == 0)
                {
                   sprintf(text, "Simple console program, support commands: help, echo, exit.\n");
                   console_str_in(text);
                }
                else if (strlen(ip) != 0)
                {
                   sprintf(text, "'%s'%s", ip, text2);
                   console_str_in(text);
                }
                else {}
