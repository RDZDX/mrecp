...
void prompt(int tid) {
    if (port == -3) {
        ...
        // dir directoryname
        ...
        } else if (vm_string_equals_ignore_case(text22, "copy") == 0 && strlen(text33) != 0 && strlen(text44) != 0 && strlen(text55) == 0) {  // newcopy
            ...
            if (lenght_555 == 0) {
                ...
                if (vm_file_get_attributes(fullPath2) != -1) {         // patikrina ar egzistuoja source failas
                    create_search_path(fullPath3, fullPath1, text44);  // suformuojam destination
                    if (vm_file_get_attributes(fullPath3) == -1) {     // patikrina kad nebutu failo/katalogo destination vardu
                ...
                if (vm_file_get_attributes(fullPath3) == VM_FS_ATTR_DIR) {        // tikrinam ar egzistuoja katalogas
                    ...
                    if (vm_file_get_attributes(fullPath4) == -1) {                // tikrinam kad nebutu destination failo/katalogo tokiu vardu
                        ...
                        if (vm_file_get_attributes(fullPath2) != -1) {            // tikrinam ar egzistuoja source failas
                            vm_file_copy(fullPath4, fullPath2, cb);               // kopijuojam copy text.txt e:/mre/
        ...
        } else if (vm_string_equals_ignore_case(text22, "move") == 0 && strlen(text33) != 0 && strlen(text44) != 0 && strlen(text55) == 0) {  // newcopy
            trim_left_symbolsx(my_path, text44);
            lenght_555 = strlen(my_path);
            lenght_666 = strlen(text44);
            strcat(text, "Name of source or destination is incorrect !\n\n");
