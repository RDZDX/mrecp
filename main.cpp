...
void prompt(int tid) {
    if (port == -3) {
        ...
        // dir directoryname
            ...
            console_str_in(text88);
            //vm_file_mkdir(const VMWSTR dirname);

        } else if (vm_string_equals_ignore_case(text22, "rmdir") == 0 &&  strlen(text33) == 0) {
            console_str_in(text88);
           //vm_file_rmdir(const VMWSTR dirname);
           //res =  vm_file_get_attributes(const VMWSTR full_path);
           //if (res == -1) {} //file or directory not found
           //if (res == VM_FS_ATTR_DIR) {} //directory
