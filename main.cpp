...
void prompt(int tid) {
    if (port == -3) {
        ...
        if (vm_string_equals_ignore_case(text22, "echo") == 0 && strlen(text33) == 0) {
            ...
            trim(text, text);
            trim_single_spec_symb(text666, text);
            strcpy(text, text666);
            strcat(text, "\n\n");
            console_str_in(text);
...
VMINT parseText2(VMSTR text) {
    ...
    if (strlen(text44X) > 0) {
       trim(text44Y, text44X); //valom tarpus
       //strncpy(text44Z, text44Y, strlen(text44Y) - 5); //nuimame ohce = echo
       ...
       //trim(text44Q, text44Q); //valom tarpus
