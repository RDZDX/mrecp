...
VMCHAR text5[113] = "cd           - change current directory\ncls          - clear screen\nexit         - quits the program\n\n";
...
void prompt(int tid) {
	...
	if (vm_string_equals_ignore_case(text1, "echo ") == 0) {
	    ...
	    strcpy(text, text7);
	    vm_ascii_to_ucs2(textw, (strlen(test3) + 1) * 2, test3);
            strcat(test3, "\\");
	    vm_ascii_to_ucs2(texty, (strlen(test3) + 1) * 2, test3);

            vm_wstrcpy(fullPath2, fullPath1);
            vm_wstrcpy(fullPath3, fullPath1);

	    vm_wstrcat(fullPath3, textw);
	    vm_wstrcat(fullPath2, texty);

	    hnd = vm_find_first_ext(fullPath3, &fileInfo);
