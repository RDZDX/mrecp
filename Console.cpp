...
int Console::scroll(int v){
	...
	if(v>0){
		...
		scroll_value+=v;
                ...
                //my_intx = scroll_value + scroll_height; //?
	        //my_intx = scroll_value + (scroll_end_row - scroll_start_row);
	        my_intx = scroll_value + (scroll_end_row - (scroll_end_row - scroll_start_row)); // neitraukia i kieki pirmojo ekrano 29 eiluciu skaiciaus !
...
void Console::clean_history(){
	terminal_w=scr_w/char_width;
	terminal_h=scr_h/char_height;

		for(int i=0; i<count_of_lines; ++i){
		    history_text[i]=(Symbol*)vm_calloc((terminal_w+1)*sizeof(Symbol));
		    for(int j=0; j<= terminal_w; ++j)
		       history_text[i][j].reset();
	        }

		//for(int i=0; i<=terminal_h; ++i){
		//    main_text[i]=(Symbol*)vm_calloc((terminal_w+1)*sizeof(Symbol));
		//    for(int j=0; j<= terminal_w; ++j)
		//	main_text[i][j].reset();
		//}
                ...
                scroll_end_row = terminal_h; // ??????????????????????????????????????????????
                scroll_value = 0; // ??????????????????????????????????????????????
                //scroll_value = scroll_value - my_intx;
}
