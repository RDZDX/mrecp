    ...
    switch (event) {
        case VM_KEY_EVENT_UP:
            switch (keycode) {
                ...
                case VM_KEY_RIGHT: {
                    lines = scr_h / char_height;  // kiek eilučių viename ekrane
                ...
                case VM_KEY_OK:
                    if (myScroll > 0) {
                        // Grąžiname ekraną į darbinę poziciją (kaip dešinys
                        // mygtukas)
                        while (myScroll > 0) {
                            myScrollx = console.scroll(1);
                            myScroll--;
                ...
                case VM_KEY_POUND:
                    switch (state) {
                        case MAIN:
                            cur_input_mode =
                                Input_mode(((int)cur_input_mode + 1) % 4);
