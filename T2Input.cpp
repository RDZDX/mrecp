...
const char * set_keyboard[12] = {"F%", "CTRL", "Alt", "TAB", "Send", "Log", "Cont", "Home", "Page Up", "Back", "End", "Page Down"};
			...
			switch(keycode){
				case VM_KEY_UP:
					//send_c("\033[A");
                                        ...
                                        if (myScrollx != 0) {myScroll = myScroll + 1;}
                                        //if (myScrollx > 0) {myScroll = myScroll + 1;}
				...
				case VM_KEY_RIGHT:
				{
 				   int lines = scr_h / char_height; // kiek eilučių viename ekrane
 				   for (int p = 0; p < lines; ++p) {
 				       if (myScroll > 0) {
  				          myScrollx = console.scroll(1);
  				          if (myScrollx != 0) myScroll--;
   				         else break;
   				     } else break;
  				  }
				}
				...
				case VM_KEY_OK:
				    if (myScroll > 0) {
 				       // Grąžiname ekraną į darbinę poziciją (kaip dešinys mygtukas)
 				       while (myScroll > 0) {
  				          myScrollx = console.scroll(1);
  				          myScroll--;
 				       }
 				       ...
 				       // Jei neslinkta – elgiamės kaip įprasta
 				       send_c("\r\n");
 				   }
				    break;
				...
				default:
					if (keycode>=VM_KEY_NUM0 && keycode<=VM_KEY_NUM9) {
 				            // Jei esame peržiūros režime, grįžtame į apačią
					    if (myScroll > 0) {
     				               while (myScroll > 0) {
       				                  myScrollx = console.scroll(1);
       				                  myScroll--;
 				            ...
 				            // Įvedame simbolį į darbinę eilutę
 				            numpad_input(keycode);
					}
					break;
			}
